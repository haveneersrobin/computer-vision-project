...
def detect(img):
    ...
    # Transform image to HSV color space. This is more suitable for this
    # feature vector seen as though the colors in the image are limited and
    # thus the differences would be too minimal.
    img_hsv = np.zeros(img.shape)
    img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    # Arrays to store all the hue, saturation and value values in.
    ...
    # We calculate the median (not the mean) of the HSV which is less prone to outliers.
    ...
    # We manually determine the lower and upper bound for
    # the HSV-values and remove all the circles not in this interval.
    for i in range(nbCircles):
